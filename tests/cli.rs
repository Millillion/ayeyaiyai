...
#[test]
fn compiles_async_generator_default_eval_arguments_body_binding_conflict() {
    let tempdir = tempdir().unwrap();
    let input = tempdir
        .path()
        .join("async-generator-default-eval-arguments-body-conflict.js");
    let output = tempdir
        .path()
        .join("async-generator-default-eval-arguments-body-conflict.wasm");
...
#[test]
fn async_function_prototype_call_preserves_this_for_completion() {
    let tempdir = tempdir().unwrap();
    let input = tempdir.path().join("async-function-call-this.js");
    let output = tempdir.path().join("async-function-call-this.wasm");
...
#[test]
fn async_unmapped_arguments_with_skipped_default_reports_completion() {
    let tempdir = tempdir().unwrap();
    let input = tempdir.path().join("async-unmapped-arguments.js");
    let output = tempdir.path().join("async-unmapped-arguments.wasm");
...
#[test]
fn async_self_has_own_restricted_properties_reports_completion() {
    let tempdir = tempdir().unwrap();
    let input = tempdir
        .path()
        .join("async-self-has-own-restricted-properties.js");
    let output = tempdir
        .path()
        .join("async-self-has-own-restricted-properties.wasm");
...
#[test]
fn async_generator_return_undefined_tick_order_reports_completion() {
    let tempdir = tempdir().unwrap();
    let input = tempdir
        .path()
        .join("async-generator-return-undefined-tick-order.js");
    let output = tempdir
        .path()
        .join("async-generator-return-undefined-tick-order.wasm");
    ...
    fs::write(
        ...
        Promise.resolve(0)
          ...
          .then(() => {
            assert.compareArray(actual, expected, "tick order");
        }).then($DONE, $DONE);
...
fn async_generator_nested_unscopables_function_replay_reports_completion() {
    ...
    let input = tempdir
        .path()
        .join("async-generator-nested-unscopables-function-replay.js");
    let output = tempdir
        .path()
        .join("async-generator-nested-unscopables-function-replay.wasm");
...

        {
          ...
          async function* ref(x) {
            ...
            (function() {
              ...
              with (globalThis) {
                count++;
                assert.sameValue(v, 10, "local v");
                v = 20;
              }
              assert.sameValue(v, 20, "updated local v");
              assert.sameValue(globalThis.v, 1, "global v");
            })();
            assert.sameValue(v, 1, "generator v");
            assert.sameValue(globalThis.v, 1, "generator global v");
            callCount = callCount + 1;
          }

          ref(10).next().then(() => {
            assert.sameValue(callCount, 1, "generator invoked once");
          }).then($DONE, $DONE);

          count++;
        }
        assert.sameValue(count, 6, "nested calls replayed");
        "#,
    )
    .unwrap();
