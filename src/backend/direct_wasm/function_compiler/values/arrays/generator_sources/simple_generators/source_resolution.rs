...
impl<'a> FunctionCompiler<'a> {
    fn simple_generator_source_environment_cache_key(&self) -> String {
        // The static-state generation counter is bumped by every mutation
        // funnel of the binding state this key used to serialize (local and
        // global value/object/array bindings), so it is a sound -- strictly
        // more conservative -- replacement for the old Debug-format of the
        // entire environment, at O(1) cost per lookup.
        crate::backend::direct_wasm::memo::static_state_generation().to_string()
    }
