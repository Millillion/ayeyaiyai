...
impl<'a> FunctionCompiler<'a> {
    fn destructuring_default_member_read<'b>(
        condition: &'b Expression,
        then_expression: &'b Expression,
        ...
        if !matches!(right.as_ref(), Expression::Undefined)
            || !static_expression_matches(left, then_expression)
        {
            return None;
        }
        let Expression::Member { object, property } = left.as_ref() else {
            return None;
        };
    ...
    pub(in crate::backend::direct_wasm) fn try_emit_destructuring_default_assign_statement(
        ...
        if let Some((object, property)) =
            Self::destructuring_default_member_read(condition, then_expression)
        {
            ...
            let temporary_local = self.allocate_temp_local();
