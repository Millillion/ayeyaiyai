...
impl<I: Tokens> Parser<I> {
    ...
    fn parse_decorator(&mut self) -> PResult<Decorator> {
        ...
        let expr = if self.input_mut().eat(Token::LParen) {
            ...
            // DecoratorMemberExpression begins with IdentifierReference[?Yield,
            // ?Await]: `await`/`yield` stay reserved only in async/generator
            // (and module) contexts, matching parse_ident_ref.
            let ctx = self.ctx();
            let expr = self
                .parse_ident(
