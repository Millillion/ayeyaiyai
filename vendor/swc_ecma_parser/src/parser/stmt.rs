...
impl<I: Tokens> Parser<I> {
    ...
    fn parse_stmt_internal(
        ...
        // `await:` in non-async script code is a LabelledStatement with the
        // identifier `await`; fall through to ordinary label parsing.
        let await_starts_label = cur == Token::Await
            && !self.ctx().contains(Context::InAsync)
            && !self.ctx().contains(Context::Module)
            && peek!(self).is_some_and(|peek| peek == Token::Colon);
        if cur == Token::Await && !await_starts_label && (include_decl || top_level) {
            if top_level {
